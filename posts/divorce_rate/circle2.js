...
var color = d3.scaleLinear()
    .domain([-1, 8])
    .range(["hsl(152,80%,85%)", "hsl(218,50%,40%)"])
...
d3.json("divorce_rate/circle_pack.json", function(error, root) {
  ...
  var circle = g3.selectAll("circle")
    ...
    .enter().append("circle")
      ...
      .style("fill", function(d) { return d.children ? color(d.depth) : d.data.color; }) // color code the different status groups
      .on("click", function(d) { if (focus !== d) zoom(d), d3.event.stopPropagation(); });

  var label = g3.selectAll("text")
    .data(nodes)
    .enter().append("text")
      .attr("class", "label")
